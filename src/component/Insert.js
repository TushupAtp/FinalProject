...
const Insert = () => {
  ...
  const validate = () => {
    ...
    } else if (food.search(/(?=.*[0-9])(?=.*[a-zA-Z]).{6,100}/) < 0) {
      document.getElementById("food").classList.add("is-invalid");
      setEplasticQty(
        "Enter description having atleast one digit, one character with minimum length of 6 and maximum of 100"
      );
    } else if (decoration.search(/(?=.*[0-9])(?=.*[a-zA-Z]).{6,100}/) < 0) {
        ...
        <form className="row g-3 w-50 m-auto bg-light p-5 fs-5">
          ...
          <div className="col-md-12">
            <input
              ...
              id="ewasteQty"
              name="ewasteQty"
              placeholder="Eg:-Full, half etc etc......"
              value={decoration}
              onFocus={clearError}
              onChange={decorationinp}
            />
          ...
          <div className="col-12 text-center mt-5 mb-5  ">
            <input
              type="button"
              value="Book"
              className="btn btn-primary btn-lg ps-3 pe-3 p-1"
              onClick={validate}
            />
          </div>
        </form>
